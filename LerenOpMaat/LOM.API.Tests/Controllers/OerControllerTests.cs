...
namespace LOM.API.Tests.Controllers
{
    public class OerControllerTests
    {
        ...
        {
            // Arrange: gebruik InMemory database
            var options = new DbContextOptionsBuilder<LOMContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString()) // unieke DB per test
                .Options;
        ...
        {
            // Arrange: bestand met verkeerde extensie en content-type
        ...
        {
            // Arrange: geldige PDF file (inhoud en extensie)
            var file = new FormFile(
        ...
        {
            // Arrange: voeg handmatig een OER toe
